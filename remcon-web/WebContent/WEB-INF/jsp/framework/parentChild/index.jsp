...
<%
	BSParentChild parentChild = (BSParentChild) session.getAttribute(HttpServletParentChild.SESSION_ATTRIBUTE_NAME_PARENT_CHILD);
...
<form id='parentForm'>
	...
	<div class="row-fluid">
		...
		<script type="text/javascript">parentFieldNames[< % = index++ % >] = "< % = field.getName() % >";</script>
		 -->
			<script type="text/javascript">parentFieldNames[<%=index++%>] = {name:'<%=field.getName()%>', desc:'<%=field.getLabel()%>', type:'<%=BSDataTypeUtil.toNormalString(field.getType())%>', value:'<%=field.getValue()== null?"": field.getValue()%>'};</script>
			<!-- 
